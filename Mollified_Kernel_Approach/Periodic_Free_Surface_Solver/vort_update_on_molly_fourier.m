function [u,xpos,zpos] = vort_update_on_molly_fourier(Xmesh,gam,mu,ep,u,gvals,L1,no_dno_term,Nvorts,Ehdt,Edt,xpos,zpos,dt,KTT)

k1 = dt*force_terms_on_molly_fourier(Xmesh,gam,mu,ep,u,gvals,L1,no_dno_term,Nvorts);
uh1 = [Ehdt*(u(1:KTT)+k1(1:KTT)/2);xpos+k1(KTT+1:KTT+Nvorts)/2;zpos+k1(KTT+Nvorts+1:KTT+2*Nvorts)/2];

k2 = dt*force_terms_on_molly_fourier(Xmesh,gam,mu,ep,uh1,gvals,L1,no_dno_term,Nvorts);
uh2 = [(Ehdt*u(1:KTT)+k2(1:KTT)/2);xpos+k2(KTT+1:KTT+Nvorts)/2;zpos+k2(KTT+Nvorts+1:KTT+2*Nvorts)/2];

k3 = dt*force_terms_on_molly_fourier(Xmesh,gam,mu,ep,uh2,gvals,L1,no_dno_term,Nvorts);
uh3 = [(Edt*u(1:KTT)+Ehdt*k3(1:KTT));xpos+k3(KTT+1:KTT+Nvorts);zpos+k3(KTT+Nvorts+1:KTT+2*Nvorts)];

k4 = dt*force_terms_on_molly_fourier(Xmesh,gam,mu,ep,uh3,gvals,L1,no_dno_term,Nvorts);
